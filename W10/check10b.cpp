...
void promptInventory(vector<Food*> &items)
{
   ...
   do
   {
      ...
      if (name != "quit")
      {
         ...
         Food* product = new Food(name, month, year); // Create a new food item using name, month, and year
         items.push_back(product); // Add it to the items vector
...
void displayInventory(vector<Food*> &items)
{
   ...
   for(vector <Food*> :: iterator fi = items.begin(); fi != items.end(); ++fi) // Use an iterator to walk through the vector
   {
      (*fi)->display(); // and call the display() method on each Food item
   }
                        ...
                        int currentYear)
{
   ...
   vector<Food*>::iterator it = items.begin(); // Use an iterator to walk through the vector
   while (it != items.end())
   {
      if ((*it)->getYear() <= currentYear && (*it)->getMonth() < currentMonth) // and detect each item that has expired. Then delete it
      {
         Food *pFood = *it;
         delete pFood;
         pFood = NULL;
         items.erase(it); // and erase it from the list.
      } else
         {
            ++it;
         }
